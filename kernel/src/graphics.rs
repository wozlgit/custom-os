...
impl Color {
    ...
    pub fn new_if_within_bounds(
        ...
        Result::Ok(Self::color_from_individual_bound_checked_consituents(
            framebuffer,
            color_values
        ))
    }
    ...
    #[inline]
    fn color_from_individual_bound_checked_consituents(
        framebuffer: &LimineFramebuffer,
        constituents: Rgb
    ) -> Color {
        Color(
...
impl LimineFramebuffer {
    ...
    fn _set_pixel_color(&self, x: u64, y: u64, color: Color) {
        ...
        let pixel_base_ptr: *mut u8 = unsafe {
            (self.address as *mut u8).add((self.get_pixel_offset(x, y) * bytes_per_pixel) as usize)
        };
        unsafe {
            for i in 0..bytes_per_pixel {
                pixel_base_ptr
                    .offset(i as isize)
                    .write_volatile(*color_p.offset(i as isize));
            }
        }
    }
    ...
    pub fn display_num(&mut self, num: u32) {
        ...
        for i in 0..num {
            ...
            for x in 0..50 {
                for y in 0..50 {
                    unsafe {
                        self.set_pixel_color_unchecked(x + pixel_offset_x, y + pixel_offset_y, color);
                    }
                }
            }
        }
    ...
    pub fn fill(&mut self, color: Color) {
        for x in 0..self.width {
            for y in 0..self.height {
                unsafe {
                    self.set_pixel_color_unchecked(x, y, color);
                }
            }
        }
    }
